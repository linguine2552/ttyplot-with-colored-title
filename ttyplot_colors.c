...
#ifndef SIGWINCH
#define SIGWINCH 28  // This is typically the correct value, but it may vary
#endif
...
static enum TitleColor title_color = TCOLOR_DEFAULT;
...
static void usage(void) {
    printf(
        ...
        "  -t title of the plot\n"
        "  -C color of the title (1-7, where 1=red, 2=green, 3=yellow, 4=blue, 5=magenta, 6=cyan, 7=white)\n"
...
static void paint_plot(void) {
    ...
    if (use_colors && title_color != TCOLOR_DEFAULT) {
...
int main(int argc, char *argv[]) {
    ...
    int cached_opterr;
    const char *optstring = "2rc:e:E:s:m:M:t:C:u:vh";
    int show_ver;
    int show_usage;
    ...
    while ((c = getopt(argc, argv, optstring)) != -1) {
        switch (c) {
            ...
            case 't':
                ...
                break;
            case 'C':
                title_color = (enum TitleColor)(atoi(optarg));
                if (title_color < TCOLOR_RED || title_color > TCOLOR_WHITE) {
                    fprintf(stderr, "Invalid color option. Using default.\n");
                    title_color = TCOLOR_DEFAULT;
                }
                break;
            case 'u':
    ...
    while ((c = getopt(argc, argv, optstring)) != -1) {
        switch (c) {
            ...
            case 't':
                ...
                break;
            case 'C':
                title_color = atoi(optarg);
                if (title_color < 1 || title_color > 7) {
                    fprintf(stderr, "Invalid color option. Using default.\n");
                    title_color = TCOLOR_DEFAULT;
                }
                break;
    ...
    if (has_colors()) {
        ...
        use_default_colors();
        for (int i = 1; i <= 7; i++) {
            init_pair(i, i, -1);  // -1 means default background
        }
    }	
